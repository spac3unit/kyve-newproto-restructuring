...
func (k msgServer) UndelegatePool(goCtx context.Context, msg *types.MsgUndelegatePool) (*types.MsgUndelegatePoolResponse, error) {
	// Unwrap context and attempt to fetch the pool.
	ctx := sdk.UnwrapSDKContext(goCtx)

	// Perform undelegation
	if err := k.Undelegate(ctx, msg.Staker, msg.PoolId, msg.Creator, msg.Amount); err != nil {
		return nil, err
	}

	// Create Unbonding queue entry
	if unbondingError := k.StartUnbondingDelegator(ctx, msg.PoolId, msg.Staker, msg.Creator, msg.Amount); unbondingError != nil {
